[toc]
## 1. 引言
通过接口MyList定义通用功能，抽象类MyAbstractList实现部分功能，MyArrayList和MyLinkedList实现类的方式进一步理解线性表。
## 2. 线性表的通用特性
...
一个好的设计策略就是在设计中提供接口和便利抽象类，以整合接口和抽象类的优点，抽象类提供饿了接口的骨架实现，用户可以认为哪个方便就用哪个。如下定义了MyArrayList和MyLinkedList通用接口。
### 2.1 MyList&lt;E>接口
```
...
```
## 3. 数组线性表
数组线性表采用数组实现，数组一旦被创建其大小是固定的，但是仍然可以通过创建更大的数组来替换当前数组实现动态的数据结构。数组类型是E[]，所以数组中每个元素实际存储的是对象的引用。
